...
#### Controller
Контроллер - гвоздь программы. Он мог бы всем управлять, но по итогу только создает и правильно инициализирует объекты всех нужных классов и запускает симуляцию методом start_simulation. Из интеремного - частицы спаунятся случайно.
```python
class Controller:
